    ...
    public class ChooseOneQuestion : QuestionBase, ICloneable
    {
        ...
        public override string ToString()
        {
            return $"{Header}      Marks({Marks}) \n{Body}\n" +
                   $"1. {AnswersList[0].AnswerText}\t\t 2. {AnswersList[1].AnswerText}\t\t 3. {AnswersList[2].AnswerText}";
        }
        ...
        public static ChooseOneQuestion AddChooseOneQuestion()
        {
            ...
            do
            {
                Console.WriteLine($"Please enter the right answer for the question [ 1, 2 or 3]");
            } while (!int.TryParse(Console.ReadLine(), out id) || id < 1 || id > 3);

            question.RightAnswer.AnswerId = id; //1
